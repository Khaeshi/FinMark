 ...
 *
 * Proves:
 * 1. Gradual traffic ramp to 200 concurrent users
 * 2. Dashboard p95 under 3 seconds (primary KPI)
 * 3. Load across microservices (Grafana multi-service graphs)
 * 4. Stable peak + graceful scale-down
 *
 * Compose / Swarm scale stack (nginx :80):
 *   k6 run -e BASE_URL=http://localhost -e AUTH_TOKEN=$token tests/load/k6/scaling-demo.js
 *
 * Do NOT use localhost:4000 for the scale stack — that is usually a local
 * single-instance gateway. Grafana watches Docker services behind nginx :80.
 *
 * Rate limits: k6 sends x-load-test: true so gateway skips per-IP limiting.
 */
...
export const options = {
  ...
  thresholds: {
    // Primary KPI only — fan-out checks are soft (asserted via check(), not thresholds)
...
export default function () {
  ...
  // ─── 1. Dashboard (report-svc — primary KPI) ──────────────────────────────
  ...
  // ─── 2. Fan-out (Grafana: order / product / admin / feedback / auth) ──────
  // Kept (decision vs Claude's dashboard-only plan): feedback/profile 500s
  // and rate-limit 429s are fixed; multi-service graphs need this traffic.
...
export function handleSummary(data) {
  ...
  const totalCache   = (hits?.values?.count || 0) + (misses?.values?.count || 0)
  const cacheHitRate = totalCache > 0
    ? ((hits?.values?.count || 0) / totalCache * 100).toFixed(1)
    : '0'

  const avgMs  = dashboard?.values?.avg?.toFixed(0) || 'N/A'
  const p95Ms  = dashboard?.values?.['p(95)']?.toFixed(0) || 'N/A'
  const p99Ms  = dashboard?.values?.['p(99)']?.toFixed(0) || 'N/A'
  const failPc = ((failures?.values?.rate || 0) * 100).toFixed(2)
  const metP95 = (dashboard?.values?.['p(95)'] || 9999) < 3000

  const summary = {
    'FinMark Scaling Demo Results': {
      'Peak Concurrent Users':     '200',
      'Dashboard Avg Response':    `${avgMs}ms`,
      'Dashboard p95 Response':    `${p95Ms}ms`,
      'Dashboard p99 Response':    `${p99Ms}ms`,
      'Dashboard Failure Rate':    `${failPc}%`,
      'Redis Cache Hit Rate':      `${cacheHitRate}%`,
      'Target Met (under 3s p95)': metP95 ? '✅ YES' : '❌ NO',
      'Services Hit':              'gateway, report, order, product, admin, feedback, user-auth',
      'Original Load Time':        '20 seconds (before optimization)',
      'Improvement':               metP95 ? `~${Math.round(20000 / (parseInt(avgMs, 10) || 1))}x faster` : 'Target not met',
    }
  }
