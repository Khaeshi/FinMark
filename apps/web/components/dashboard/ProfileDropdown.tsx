...
import { ProfileActionModal, type ProfilePanel } from './ProfileActionModal'

const MENU_ITEMS: { icon: typeof User; label: string; desc: string; panel: ProfilePanel }[] = [
  { icon: User,       label: 'View Profile',    desc: 'Personal info & avatar',      panel: 'profile' },
  { icon: Bell,       label: 'Notifications',   desc: 'Alerts & preferences',        panel: 'notifications' },
  { icon: KeyRound,   label: 'Change Password', desc: 'Security settings',           panel: 'password' },
  { icon: Palette,    label: 'Appearance',      desc: 'Theme & display',             panel: 'appearance' },
  { icon: Shield,     label: 'Security & 2FA',  desc: 'Authentication methods',      panel: 'security' },
  { icon: HelpCircle, label: 'Help & Support',  desc: 'Docs & contact',              panel: 'help' },
]
...
export function ProfileDropdown() {
  ...
  function openPanel(p: ProfilePanel) {
    setOpen(false)
    setPanel(p)
  }
  ...
  return (
    <>
      <div className="relative" ref={ref}>
        ...
        {open && (
          <div className="absolute right-0 top-full mt-2 w-72 rounded-2xl border border-white/10 bg-[#141820] shadow-2xl shadow-black/40 z-50 overflow-hidden">
            ...
            <div className="py-1">
              {MENU_ITEMS.map(({ icon: Icon, label, desc, panel: p }) => (
                <button
                  key={label}
                  onClick={() => openPanel(p)}
      ...
      {panel && (
        <ProfileActionModal panel={panel} onClose={() => setPanel(null)} />
      )}
    </>
  )
}
